...
public final class TestData {
    ...
    public static EmailTemplate template(Long id, String code) {
        return new EmailTemplate(id, code, "Successful Delivery - FA", "FA success email",
                "STAAT Client Retention Packs", "<p>Hi {faName}</p>", true,
                "muru", "muru", NOW, NOW);
    }

    /** A single-part FA send that succeeded on its first attempt. */
    public static EmailLog sentLog() {
        return new EmailLog(42L, null, 1L, 2L, "FA-42", null, "fa42@ubs.com",
                List.of("ACE-1001"), Map.of("faName", "Alex Advisor"),
                "STAAT Client Retention Packs", "<p>Hi Alex Advisor</p>",
                List.of("John_Smith_Household.html"), 1, 1, false, "sme-abc",
                EmailStatus.SENT, 1, NOW, NOW, null, "system", "system", NOW, NOW);
    }

    /** A persisted-looking Field-Leader batch entry in the given state. */
    public static EmailBatch batch(BatchStatus status) {
        return new EmailBatch(11L, null, "FL-7", "fl7@ubs.com", 2L, 3L, List.of("ACE-1003"),
                Map.of("fieldLeaderName", "Pat Leader"), SCHEDULED_AT, status, null, null,
                "muru", "muru", NOW, NOW);
    }
    ...
    public static StaatInsightDocument insightDocument(String aceId, String leadClientName) {
        return new StaatInsightDocument(1L, aceId, leadClientName,
                leadClientName.replace(' ', '_') + ".html", "<html><body>pack</body></html>",
                NOW, NOW);
    }
}
