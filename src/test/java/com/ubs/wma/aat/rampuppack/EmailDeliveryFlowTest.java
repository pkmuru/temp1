...
class EmailDeliveryFlowTest extends AbstractIntegrationTest {
    ...
    void sendDeliversAndStoresMergedContent() {
        ...
        assertThat(part.attachmentFileNames())
                .containsExactly("John_Smith_Household.html", "Jane_Doe_Household.html");
    ...
    void failedSendRecordsFailureAndTriggersOneNotice() {
        ...
        EmailSendResponse response = postSend(
                sendRequest(templateId, failTemplateId, "FA-KO", List.of("ACE-1001")))
                .expectStatus().isOk()
                .expectBody(EmailSendResponse.class)
                .returnResult().getResponseBody();
        ...
        List<EmailLog> notices = emailLogRepository.findAll()
    ...
    void previewMergesLikeSendButDeliversNothing() {
        ...
        EmailPreviewResponse preview = webTestClient.post().uri("/api/v1/emails/preview")
        ...
        Long loggedRows = emailLogRepository.findAll()
    ...
    void dueBatchIsClaimedProcessedAndCompletedByTheScheduler() {
        ...
        EmailBatchResponse created = webTestClient.post().uri("/api/v1/email-batches")
                .bodyValue(new EmailBatchRequest(List.of("ACE-1003"), null, "FL-7",
                        templateId, failTemplateId, "fl7@ubs.com",
                        Map.of("faName", "Pat Leader"), Instant.now().minusSeconds(60)))
                .exchange()
        ...
        List<EmailLog> batchLogs = emailLogRepository.findByBatchId(created.id()).collectList().block();
    ...
    void failedDeliveryIsRetriedByTheSchedulerUntilItSucceeds() {
        ...
        EmailSendResponse response = postSend(
                sendRequest(templateId, failTemplateId, "FA-RETRY", List.of("ACE-1002")))
                .expectStatus().isOk()
                .expectBody(EmailSendResponse.class)
                .returnResult().getResponseBody();
    ...
    private EmailDeliveryScheduler scheduler() {
        return new EmailDeliveryScheduler(emailBatchRepository, emailLogRepository, emailSendService,
                new SchedulerProperties(true, Duration.ofHours(1), 25), emailProperties);
    }
    ...
    private void stubStaatEmailHealthy() {
        ...
        willReturn(Mono.just(new StaatSendResponse("sme-ok", "ACCEPTED")))
                .given(staatEmailClient).sendEmail(any());
    }
    ...
    private void stubUploadOk() {
        ...
        }).given(staatEmailClient).uploadAttachment(any());
    }

    private Long createTemplate(String code) {
        EmailTemplateRequest request = new EmailTemplateRequest(code, code, null,
                "Packs for {faName}", "<p>Hi {faName}, {packCount} pack(s).</p>{householdTable}", null);
    ...
    private static EmailSendRequest sendRequest(Long templateId, Long failTemplateId,
                                                String faId, List<String> aceIds) {
        return new EmailSendRequest(aceIds, faId, null, templateId, failTemplateId,
                "fa@ubs.com", Map.of("faName", "Alex Advisor"));
    }

    private WebTestClient.ResponseSpec postSend(EmailSendRequest request) {
        return webTestClient.post().uri("/api/v1/emails/send")
                .bodyValue(request)
                .exchange();
    }
}
