...
class EmailTemplateRepositoryTest extends RepositoryTestSupport {
    ...
    void saveAssignsIdAndAuditColumns() {
        EmailTemplate template = EmailTemplate.newTemplate(
                "REPO_TPL_AUDIT", "Audit Test", null, "Subject {faName}", "<p>{householdTable}</p>", true);
    ...
    void findByActiveFilters() {
        EmailTemplate active = repository
                .save(EmailTemplate.newTemplate("REPO_TPL_ACTIVE", "Active", null, "s", "b", true))
                .block();
        EmailTemplate inactive = repository
                .save(EmailTemplate.newTemplate("REPO_TPL_INACTIVE", "Inactive", null, "s", "b", false))
                .block();

        var activeCodes = repository
                .findByActive(true)
                .map(EmailTemplate::code)
                .collectList()
                .block();
        var inactiveCodes = repository
                .findByActive(false)
                .map(EmailTemplate::code)
                .collectList()
                .block();
    ...
    void duplicateCodeIsRejectedByUniqueConstraint() {
        repository
                .save(EmailTemplate.newTemplate("REPO_TPL_DUP", "First", null, "s", "b", true))
                .block();

        StepVerifier.create(repository.save(EmailTemplate.newTemplate("REPO_TPL_DUP", "Second", null, "s", "b", true)))
