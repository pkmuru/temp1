...
class EmailSendServiceSplitTest {
    ...
    void keepsEverythingInOnePartWhenUnderTheCap() {
        List<List<StaatInsightDocument>> parts = EmailSendService.splitBySize(
                List.of(document("A", 10), document("B", 20), document("C", 30)), 100);
    ...
    void startsANewPartWhenTheCapWouldBeExceeded() {
        List<List<StaatInsightDocument>> parts = EmailSendService.splitBySize(
                List.of(document("A", 40), document("B", 50), document("C", 30)), 100);
    ...
    void shipsAnOversizedDocumentAloneInItsOwnPart() {
        List<List<StaatInsightDocument>> parts = EmailSendService.splitBySize(
                List.of(document("A", 10), document("HUGE", 150), document("B", 10)), 100);
    ...
    private static StaatInsightDocument document(String aceId, int sizeBytes) {
        return new StaatInsightDocument(null, aceId, "Client " + aceId, aceId + ".html",
                "x".repeat(sizeBytes), null, null);
    }
