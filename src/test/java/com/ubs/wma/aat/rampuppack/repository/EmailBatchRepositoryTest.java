...
class EmailBatchRepositoryTest extends RepositoryTestSupport {
    ...
    void jsonbMergeFieldsAndAceIdArrayRoundTrip() {
        EmailBatch saved = repository.save(newBatch("BATCH_REPO_RT",
                Instant.now().plus(1, ChronoUnit.DAYS))).block();
    ...
    void claimDueClaimsOnlyDueScheduledRows() {
        EmailBatch due = repository.save(newBatch("BATCH_REPO_DUE",
                Instant.now().minus(1, ChronoUnit.HOURS))).block();
        EmailBatch future = repository.save(newBatch("BATCH_REPO_FUTURE",
                Instant.now().plus(1, ChronoUnit.DAYS))).block();
        EmailBatch cancelled = repository.save(repository.save(newBatch("BATCH_REPO_CANCELLED",
                Instant.now().minus(1, ChronoUnit.HOURS))).block().asCancelled()).block();
        ...
        List<Long> secondClaim = repository.claimDue(100).map(EmailBatch::id).collectList().block();
    ...
    private EmailBatch newBatch(String templateCode, Instant scheduledAt) {
        ...
        return EmailBatch.scheduled("FA-1", null, "fa1@ubs.com", template.id(), template.id(),
                List.of("ACE-1001", "ACE-1002"),
                Map.of("faName", "Alex Advisor", "FA Name", "Alex Advisor"), scheduledAt);
    }
}
