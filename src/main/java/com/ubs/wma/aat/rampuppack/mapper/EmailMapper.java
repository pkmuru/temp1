...
public final class EmailMapper {
    ...
    public static EmailSendResponse toSendResponse(List<EmailLog> parts) {
        return new EmailSendResponse(
                parts.stream().map(EmailMapper::toPartResult).toList());
    }
    ...
    public static EmailPreviewResponse toPreviewResponse(String recipientEmail, List<EmailLog> plannedRows) {
        List<EmailPreviewPart> parts = plannedRows.stream()
                .map(row -> new EmailPreviewPart(
                        row.partNumber(),
                        row.totalParts(),
                        row.mergedSubject(),
                        row.mergedBody(),
                        row.aceIds(),
                        row.attachmentFileNames()))
                .toList();
    ...
    public static EmailLogResponse toResponse(EmailLog emailLog) {
        return new EmailLogResponse(
                ...
                emailLog.batchId(),
                emailLog.templateId(),
                emailLog.failTemplateId(),
                emailLog.faId(),
                emailLog.fieldLeaderId(),
                emailLog.recipientEmail(),
                emailLog.aceIds(),
                emailLog.mergeFields(),
                emailLog.mergedSubject(),
                emailLog.mergedBody(),
                emailLog.attachmentFileNames(),
                emailLog.partNumber(),
                emailLog.totalParts(),
                emailLog.failureNotification(),
                emailLog.smeId(),
                emailLog.status(),
                emailLog.attemptCount(),
                emailLog.firstAttemptedAt(),
                emailLog.lastAttemptedAt(),
                emailLog.failureReason(),
                emailLog.createdBy(),
                emailLog.createdAt(),
                emailLog.updatedAt());
